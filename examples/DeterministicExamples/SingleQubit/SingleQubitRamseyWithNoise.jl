...
plot_callback = function(_trace)
	p = _trace.metadata["x"]
	rho_ramsey_u,_ = GLOQ.RamseyForwardSolve(state_u0,state_v0,
					 (2*pi).*[p[1]],omr,
					 gamma1,[p[2]],#gamma1,gamma2,
					 initial_state, # initial state
					 TC,t_dark_times,N_states)
	...
	plot!(fig,t_dark_times./GLOQ.GLOQ_MICRO_SEC,population_ramsey,label=["Optim-0" "Optim-1"]
...
println("\nOptim results: ",sol.minimizer,
		...
		"\nOptim error: ",sol.minimizer-p_true)
...
println("\nNLopt results: ",loss_value_nlopt,
        "\nNLopt loss: ",sol_nlopt,
		"\nNLopt error: ",sol_nlopt-p_true)
...
rho_ramsey_u,_ = GLOQ.RamseyForwardSolve(state_u0,state_v0,
					 ...
					 TC,t_dark_times,N_states)
population_ramsey = GLOQ.get_population(rho_ramsey_u)
fig_nlopt=plot(t_dark_times./GLOQ.GLOQ_MICRO_SEC,noisy_data,label=["Noisy-0" "Noisy-1"],
			 line = (:dash,0.0), marker = ([:hex :hex], 5, 0.5)  )
plot!(fig_nlopt,t_dark_times./GLOQ.GLOQ_MICRO_SEC,population_ramsey,label=["NLopt-0" "NLopt-1"]
		 )			#,line = (:dot, 4), marker = ([:hex :hex], 5, 0.1))
display(fig)