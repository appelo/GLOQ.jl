## Example 2: characterization of single qudit Ramsey experiment, Echo experiment and T1-decay experiment.
In this example, we solve a characterization problem based on the synthetic data of a  Ramsey experiment, an Echo experiment and a T1-decay experiment of a single qubit system. We seek the decay time $T_1=1/\gamma_1$, the dephasing time $T_2=1/\gamma_2$ and the transition frequency $\omega_{01}$. In this example, there is no noise in the synthetic data.
This example is corresponding to `GLOQ.jl/examples/SingleQubitBayesian.jl`. 
The code can be invoked by e.g. `cd("examples");include("SingleQubitBayesian.jl")`. 
...
#### Generate synthetic data for the Ramsey experiment with a forward solve
```julia
...
```
#### Generate sythetic data for the Echo experiment with a forward solve
```julia
# Echo experiment
...
```
#### Generate synthetic data for the T1-decay experiment with a forward solve
```julia
# T1
...
```math
	\big|\big| \textrm{Forward Solve Ramsey Results}-\textrm{Synthetic Ramsey Data}\big|\big|_2^2\Delta t_{\textrm{Ramsey}}+
	\big|\big| \textrm{Forward Solve Echo Results}-\textrm{Synthetic Echo Data}\big|\big|_2^2\Delta t_{\textrm{Echo}}
	\big|\big| \textrm{Forward Solve T1-decay Results}-\textrm{Synthetic T1-decay Data}\big|\big|_2^2\Delta t_{T_1}
```
with $||\cdot||_2$ being the $l_2$-norm, $\Delta t_{\textrm{Ramsey}}$ is the time step size of the Ramsey experiment, $\Delta t_{\textrm{Echo}}$ is the time step size of the Echo experiment, and $\Delta t_{\textrm{T_1}}$ is the time step size of the $T_1$-decay experiment.
```julia
